...
- مدل ماندگاری: **داده‌ها** (`/etc/x-ui` شامل `x-ui.db`) کاملاً ماندگارند و در هر اسنپ‌شات ذخیره می‌شوند؛
  باینری حجیم (~200MB) وارد آرشیو نمی‌شود و در هر ریبیلد، چون داده وجود دارد، توسط `provision.sh`
  به‌طور خودکار بازنصب می‌شود (recovery) — یعنی پنل با همان تنظیمات/اکانت‌ها بالا می‌آید.
...
## مصرف دقیقهٔ Actions و محدودیت‌های واقعی
ریپو **public** است → روی پلن Free، اجرای Actions برای ریپوهای عمومی **نامحدود و رایگان** است
(سقف ۲۰۰۰ دقیقه/ماه مربوط به ریپوهای **خصوصی** است). ریپوی state خصوصی است ولی Actions روی آن
اجرا نمی‌شود، پس مصرفی ندارد.

محدودیت‌های واقعی این معماری:
- **حداکثر زمان هر job: ۶ ساعت** → دلیل عمر ۳۵۰ دقیقه‌ای هر ران و زنجیره‌ی جانشین.
- **سقف اجرای همزمان (concurrency)** پلن Free و تأخیر زمان‌بند کرون گیت‌هاب (گاه چند دقیقه تا بیش از یک ساعت).
- **انقضای توکن‌ها** (PAT / Tailscale) و اختلال‌های موقت سرویس Actions.

برای مقابله با این‌ها: زنجیره‌ی جانشین خودکار + دو نگهبان (watchdog) هر ۱۰ دقیقه + کرن ساعتی به‌عنوان backstop.
